...
class LinkedListNode[A](var value: A, var next: LinkedListNode[A]) {
  ...
  // Writing the Monday LQ
  def recursiveReduce(node: LinkedListNode[A],foo:(A,A) => A): A ={
    if(node.next == null){
      return node.value
    }
    else{
      // Tail recursive accumulation of this node's value with the next node's value
      foo(node.value,recursiveReduce(node.next,foo))
    }
  }

  //Use recursion
  def reduce(foo: (A,A) => A): A ={
    if (this.next == null){
      return this.value
    }
    else {
      recursiveReduce(this, foo)
    }
  }
